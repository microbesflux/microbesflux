package edu.wustl.client;

import edu.wustl.shared.FieldVerifier;
 ...
 */
public class DFBA implements EntryPoint {

	private Anchor h_start = new Anchor("Start", "#");
	private Anchor h_model = new Anchor("Model", "#");
	private Anchor h_optimize = new Anchor("Optimization", "#");
	private Anchor h_result = new Anchor("Result", "#");

	private Label lbl = new Label();
	HorizontalSplitPanel hsp;
	String[] hints = { "hsa", "ptr", "mcc", "mmu", "rno", "cfa", "bta", "ssc",
	...
	String[] realname = { "H.sapiens", "P.troglodytes", "M.mulatta",
	...
	public void onModuleLoad() {
		...
		h_start.addClickHandler(new ClickHandler() {
			public void onClick(final ClickEvent event) {
				changeToStart();
			}
		});

		h_model.addClickHandler(new ClickHandler() {
			public void onClick(final ClickEvent event) {
				changeToModel();
			}
		});

		h_optimize.addClickHandler(new ClickHandler() {
			public void onClick(final ClickEvent event) {
				changeToOptimization();
			}
		});

		h_result.addClickHandler(new ClickHandler() {
			public void onClick(final ClickEvent event) {
				changeToResult();
			}
		});

		hsp.setSize("700px", "550px");
		hsp.setSplitPosition("20%");
		hsp.setLeftWidget(vp);

		hsp.setRightWidget(new HTML("Hello"));
		DecoratorPanel dp = new DecoratorPanel();
		dp.add(hsp);
		RootPanel.get("dfbaroot").add(dp);
	}

	private void selected(Anchor a) {
		a.addStyleName("Link-selected");
	}

	private void removeFormat() {
		h_start.removeStyleName("Link-selected");
		h_model.removeStyleName("Link-selected");
		h_optimize.removeStyleName("Link-selected");
		h_result.removeStyleName("Link-selected");
	}

	private void changeToStart() {
		removeFormat();
		hsp.setRightWidget(box);
		selected(h_start);
	}
	...
	private void changeToModel() {
		...
		hsp.setRightWidget(null);
	}

	private void changeToOptimization() {
		removeFormat();
		selected(h_optimize);

		hsp.setRightWidget(new HTML("In optimization"));
	}
	...
	private void changeToResult() {
		...
		hsp.setRightWidget(new HTML("In result"));
	}
