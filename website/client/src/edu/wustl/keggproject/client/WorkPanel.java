...
//			}
	...
	public void initializeLeft(){
		...
		paneLink.setPosition("Center"); // not quite understand how setPosition works
	...
	protected void changeToOptimization() {
		...
		*/
		...
		constraintList.setDataSource(ConstraintDS.getInstance()); //data source for constrainList
		ListGridField compound = new ListGridField("c"); //term in data source
		ListGridField constraint   = new ListGridField("r"); // term in data source
		...
		boundarySave.addClickHandler(new ClickHandler(){
			public void onClick(ClickEvent event){
				float l, u;
				try{
					l = Float.valueOf(boundaryForm.getValueAsString("l")).floatValue();
					u = Float.valueOf(boundaryForm.getValueAsString("u")).floatValue();
				}
